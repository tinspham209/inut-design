...
license: MIT
compatibility: Requires git. The project must be a git repository. Requires terminal access to run commands. Uses pnpm (not npm/yarn).
metadata:
  author: luiscantero
  adapted-for: inut-design (Next.js 12 + Sanity + MUI + Zustand)
...
- `components/` — UI components (memoization, layout, rendering)
- `styles/` — CSS-in-JS, MUI theme, global styles
- `pages/` routes (except `pages/checkout/`)
- `api-client/` GROQ queries (tighter projections, fewer fields)
- `utils/` (except `analytics.ts`, `priceCalculator.ts`)
- `next.config.js` — webpack chunking, image config, headers
- TypeScript and import structure
...
- `utils/analytics.ts`, `utils/umamiAnalytics.ts`
...
> - TypeScript compatibility (this project uses `strict: false` — avoid strict-only patterns)
> - Code complexity limits (prefer simpler solutions)

**Inut Design baseline constraints** (always apply unless user overrides):

- Use `pnpm` only. Never run `npm install` or `yarn`.
- All Sanity array writes must preserve `_key` on every item.
- Do not remove analytics tracking events.
- TypeScript must stay compatible with `strict: false`.
...
- **Bundle size**: dynamic `import()` lazy loading for heavy MUI components, `next/dynamic` for modal/drawer content, tree-shake lodash (`lodash-es` or named imports), reduce barrel exports.
- **Build time**: narrow TypeScript `include` paths, smaller webpack `externals`, remove unused `@react-three` chunks from initial load.
- **Runtime perf**: memoize expensive components with `React.memo`, stabilize Zustand selectors, avoid over-rendering in cart updates.
- **GROQ queries**: reduce projection fields to only what each page uses, add `order()` for consistent results, limit array slices.
- **SEO/Lighthouse**: defer non-critical MUI icons, optimize `next/image` sizes, strip unused font weights.
- **Code quality**: reduce lint errors, consolidate duplicate utility functions.
...
4. **Run final verification** (Inut Design):
   ```
   ...
   ```
5. **Recommend next steps**: Based on the results, suggest what a human researcher might try next (ideas that were too risky/complex for automated experimentation).
